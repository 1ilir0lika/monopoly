...
# ---------------------------------------------------------------------------
# Overlay card (Imprevisto / Probabilita / Tax / Jail)  –  async
# ---------------------------------------------------------------------------

async def draw_overlay_card(title, body, accent=(200, 60, 40), icon_txt='?!'):
...
# ---------------------------------------------------------------------------
# Buy-property prompt  –  async
# ---------------------------------------------------------------------------

async def draw_buy_prompt(prop, player):
    bx, by = set_board.board_x, set_board.board_y
    bw     = set_board.board_size
...
def _draw_sidebar(current_player):
    ...
    card_x   = sx + 16
    card_w   = sw - 32
    total_h  = sh - sep_y - 18 - spacing
    card_h   = (total_h - spacing * (n - 1)) // n
    start_y  = sep_y + 18
    ...
    FIXED_H  = 8 + nf.get_height() + 3 + cf.get_height() + 6
    prop_area = card_h - FIXED_H - 8
...
# ---------------------------------------------------------------------------
# Dice animation  –  async
# ---------------------------------------------------------------------------

async def _animate_dice(result):
    bx, by = set_board.board_x, set_board.board_y
    bs     = set_board.board_size
    size   = int(bs * 0.17)
    cx     = bx + bs // 2
    ...
    for frame in range(20):
        ...
        # asyncio.sleep instead of pygame.time.wait
        await asyncio.sleep(0.045 if frame < 16 else 0.11)
...
def _apply_card_effect(player, effect):
    ...
    if effect.get("cash_pct"):
        ...
        if effect["cash_pct"] < 0:
            ...
            player.cash += amount
            msgs.append(f"Guadagni il {int(effect['cash_pct']*100)}% in piu': +{amount}$.")
    ...
    match type(cell):
        case classi.Property:
            if cell.owner is None and cell.price > 0:
                if await draw_buy_prompt(cell, player):
                    cell.buy(player)
        ...
        case classi.Tax:
            ...
            await draw_overlay_card("TASSA", f"Hai pagato {cell.amount}$ di tasse.",
